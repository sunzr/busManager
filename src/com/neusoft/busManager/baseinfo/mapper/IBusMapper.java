...
public interface IBusMapper {
	 //增加车辆信息
		...
		//按条件取得所有车辆信息
			public List<BusModel> selectListByCondition(@Param("typeno")int typeno,@Param("factoryno") int factoryno) throws Exception;
		//分页方式取得所有车辆信息
			public List<BusModel> selectListByAllWithPage(RowBounds rb)throws Exception;
			
			//按检索条件取得所有车辆信息，分页方式
			public List<BusModel> selectListByConditionWithPage(@Param("typeno")int typeno,@Param("factoryno") int factoryno,RowBounds rb) throws Exception;		
		//取得车辆信息的个数
